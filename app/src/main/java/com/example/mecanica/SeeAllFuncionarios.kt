...
class SeeAllFuncionarios : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            val cursor: Cursor = database.rawQuery("SELECT * FROM funcionarios", null)

            // Índices das colunas
            val nameIndex = cursor.getColumnIndex("nome")
            val rgIndex = cursor.getColumnIndex("rg")
            val nascIndex = cursor.getColumnIndex("contrato_data")
